  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {passwordFocused && (
          <div>
            <p>{passwordConditions.length ? '✓' : '✗'} 비밀번호 8자리 이상(필수)</p>
            <p>{passwordConditions.number ? '✓' : '✗'} 숫자 포함</p>
            <p>{passwordConditions.upper && passwordConditions.lower ? '✓' : '✗'} 영문 대소문자 포함</p>
            <p>{passwordConditions.special ? '✓' : '✗'} 특수문자 포함</p>
            <p>{passwordConditions.noSequential ? '✓' : '✗'} 연속된 문자열이나 숫자가 없어야 함</p>
          </div>
        )}
        <div>
          <label>비밀번호 확인</label>
